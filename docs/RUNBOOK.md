...
| Tick summary `payouts.halted: true` | The payout worker signs nothing; claims wait as `queued` | §5.5 — never "just resume" |
| A claim is `needs_review` | Treasury halted automatically | §5.5 |
| `payouts.stuck > 0` | Worker re-broadcasts the persisted tx every tick | Nothing for an hour; then §5.6 |
| `treasury_status` shows TTWO < 0.75 or ETH < 0.005 | Claims wait as `queued`, reason reported | §5.2 refill from cold |
| `payouts.reason: insufficient_*` / `gas_above_ceiling` | Nothing signed, nothing lost | Refill, or wait out the gas spike |
...
### 5.1 Treasury key — generate and push (once)

The treasury pays TTWO rewards. Its key must never pass through an AI session, a chat, a command
line, a clipboard manager or a file inside the repo. Do this in **your own Terminal.app**, not in
Claude Code (not even with `!`) — anything shown there is transcribed. The design and the reasons are
in `docs/CONTRACTS-PREDICTIONS.md` §10.

1. `mkdir -p ~/.foundry/keystores && chmod 700 ~/.foundry/keystores`
2. Rehearse with a throwaway: `cast wallet new "$(mktemp -d)" throwaway` — type any passphrase at the
   hidden prompt. The output must show an `Address:` and **no private key**. If it shows a key, stop.
3. The real one: `cast wallet new ~/.foundry/keystores wanted-treasury`. Passphrase: generated by
   your password manager, 24+ characters, typed only at the hidden prompt. Never `--unsafe-password`,
   never `--password <value>`, never `CAST_PASSWORD=` — all three put it on a command line or in the
   environment. Note the address — cast's keystore file does not record it, so this line and step 4
   are what bind the file to the address. Then `chmod 600 ~/.foundry/keystores/wanted-treasury`:
   cast writes the file world-readable (0644).
4. Prove it decrypts: `cast wallet address --keystore ~/.foundry/keystores/wanted-treasury` (hidden
   prompt) must print the same address.
5. Back it up as **two separate** password-manager items: *WANTED treasury keystore* (the file
   attached) and *WANTED treasury passphrase*. Restore test: copy the file out to a temp path, run
   step 4 against it, delete the copy.
6. Push to Vercel, dry run first:
   `cd web && node scripts/treasury-push-key.mjs --keystore ~/.foundry/keystores/wanted-treasury --expect 0x<address> --dry-run`
   then the same without `--dry-run`. It asks for the passphrase, checks the keystore MAC and that the
   key's address matches `--expect`, then makes two pushes on stdin: `TREASURY_PRIVATE_KEY` to
   Production as Sensitive, and `NEXT_PUBLIC_TREASURY_ADDRESS` to Production (public, `--no-sensitive`),
   both with `--force` so the same command also serves a rotation. Stdout is only the address;
   Vercel's status (with any 64-hex run redacted) goes to stderr.
7. `npx vercel env ls production` must list `TREASURY_PRIVATE_KEY` and `NEXT_PUBLIC_TREASURY_ADDRESS`;
   `npx vercel env ls preview` and `... development` must list **neither**.
8. Env vars are baked into a deployment, so redeploy production, then prove the deployed worker holds
   the right key: an authenticated `GET /api/cron/tick` must report `payouts.treasury` equal to the
   address from step 3. A different address means the wrong key was pushed — stop, and repeat step 6.

Banned, always: `cast wallet decrypt-keystore` (prints the key), `--private-key` anywhere, pasting
the key anywhere, putting it in `web/.env.local`, `harness/.env`, `infra/.env.cloud` or the game server.

### 5.2 Treasury funding and refills

Hot wallet (the one above): ceiling **1.75 TTWO** and **0.02 ETH**; everything else stays in your
cold wallet, which no code knows. First tranche for the go-live checks: **0.05 TTWO + 0.005 ETH**.
Refill when the tick's `treasury_status` shows TTWO < 0.75 or ETH < 0.005, or `runway_days` < 3:
send from cold only up to the ceiling, confirm the transfer on robinhoodchain.blockscout.com, and
check the next tick shows it. Never sweep cold into hot in one go, never top up past the ceiling.

The two switches, both instant, no deploy — `rewards.enabled` (new credits) and `rewards.payouts`
(signing and broadcasting):

```sql
insert into public.site_config (key, value) values ('rewards', '{"payouts": false}')
on conflict (key) do update set value = public.site_config.value || excluded.value;
```

### 5.3 Treasury key rotation (planned, ~15 min)

1. `payouts` → false (SQL above). 2. Wait until the tick reports `in_flight: 0`; resolve any stuck
claim first (§5.6) — a signed tx from the old key stays valid forever. 3. Generate the new keystore
(§5.1, name `wanted-treasury-<yyyymmdd>`), push it with `--expect <new address>`. 4. `npx vercel
redeploy` production. 5. From the Mac, sweep TTWO then ETH (minus gas) from the old key to the new
address with `cast send --keystore <old keystore> ...`. 6. The worker initialises the new address's
nonce from the chain on its next idle tick. 7. `payouts` → true. 8. Rename the old password-manager
items `RETIRED-<date>`; keep them.

### 5.4 Treasury compromise (minutes matter)

Signals: the tick reports `halt_reason: unrecorded_tx_from_treasury`, a claim goes `needs_review`
with a nonce nobody recorded, or the hot balance drops with no matching claim row.

1. `payouts` → false. 2. **Sweep** every TTWO and ETH from the hot key to the cold wallet from the
Mac, before anything else — it is a race and the sweep is the only move that wins it. 3. Rotate
(§5.3) into a fresh key; fund from cold only to the ceiling. 4. Assume the whole production env
leaked: rotate `SUPABASE_SECRET_KEY`, `CRON_SECRET`, `SESSION_SECRET`, reissue Vercel tokens, review
team members and integrations. 5. List every outbound tx from the old address with no
`reward_claims` row in `docs/STATUS.md`. 6. Only then re-enable. Maximum loss by construction: the
hot ceiling (1.75 TTWO + 0.02 ETH).

### 5.5 A halted treasury or a `needs_review` claim

The worker halts when a nonce was consumed by a transaction it did not record. It will not resume by
itself, and code must never resume it.

```sql
select id, wallet, nonce, tx_hash, override_tx_hash, override_kind, error
  from public.reward_claims where status = 'needs_review';
select * from public.treasury_accounts;
```

Open the treasury address on Blockscout and find the transaction at that nonce.
- It is this claim's `tx_hash` (or a `pay` override) with status success →
  `select public.resolve_claim_review('<id>', 'confirmed', '<tx hash>', <block>);`
- The viewer was **not** paid (a cancel, or the tx reverted) →
  `select public.resolve_claim_review('<id>', 'failed', null, null);` — their credits return.
- It is a transaction **nobody on the team sent** → §5.4 first. Do not resume.

If the halt reason is `unrecorded_tx_from_treasury` (the idle check saw the key used outside the
system), the database's next nonce is now behind the chain, and resuming alone halts again on the next
tick. Only after §5.4 has settled whose transaction it was — and the key is rotated if it was not
yours — align it with the chain, never below a nonce a recorded claim already used:

    cast nonce <treasury address> --rpc-url https://rpc.mainnet.chain.robinhood.com

```sql
update public.treasury_accounts
   set next_nonce = <that count>, updated_at = now()
 where address = '<treasury address>'
   and <that count> >= coalesce((select max(nonce) + 1 from public.reward_claims
                                 where signer_address = '<treasury address>'), 0);
```

It must report `UPDATE 1`. `UPDATE 0` means the count is below a nonce a recorded claim used — stop.

Then `select public.resume_payouts('<treasury address>');`

### 5.6 Cancelling a stuck claim

Only when a claim has been `broadcast` with no receipt for over an hour and is blocking the queue
(payouts are strictly in nonce order).

1. `payouts` → false.
2. From the Mac, consume its nonce with a 0-value self-transfer (hidden passphrase prompt):
   `cast send <treasury address> --value 0 --nonce <claim nonce> --keystore ~/.foundry/keystores/wanted-treasury --rpc-url https://rpc.mainnet.chain.robinhood.com`
3. Check it is what you meant before recording it: `cast tx <that tx hash> --rpc-url
   https://rpc.mainnet.chain.robinhood.com` must show `from` = the treasury address and `nonce` = the
   claim's nonce. The worker re-checks both and sends a mismatched override to `needs_review` instead
   of honouring it — a cancel at the wrong nonce would return credits for a transfer that can still land.
   Then `select public.record_claim_override('<claim id>', '<that tx hash>', 'cancel');`
4. `payouts` → true. The next tick sees the cancel's receipt, fails the claim with proof
   `cancel_receipt`, and returns the viewer's credits.

To push the payment through instead, send the same `transfer` at the same nonce and record it with
kind `pay`; the worker then confirms the claim against that hash.
