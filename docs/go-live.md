...
| T8 phone | fix-sonnet-d, amended live | **Jobs off: every ring is rejected and a call that got connected is hung up at once** (live 2026-09-03 12:29Z: an answered Simeon call froze him for its whole length — `phone.in_call` true, control on, every movement task at 0.00 m — and the stuck watchdog then killed the goal; a call is now also a movement hold for the goal engine, the stranded reflex and the stillness clock). Jobs on: answer, never hang up. One attempt per ring / per call; bridge tries control group 0 then 2 and logs which worked; `DESTROY_MOBILE_PHONE` if the handset is up with no ring/call for 10 s. **Bug fixed:** `answer_call`/`reject_call` were missing from `bridge_client.BRIDGE_TASK_TYPES` — the literal cause of "it cant cut the call". |
